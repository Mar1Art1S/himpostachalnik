        ...
        <div>
            ...
            @foreach ($product->getTranslation('options', app()->getLocale()) as $options)
                <div class="flex flex-row items-center w-full px-4 mt-4 lf:mt-2 border-b border-slate-100 pb-2 last:border-b-0">
                    <div class="justify-start w-3/4 text-sm text-[#666] text-start lf:leading-[15px] lf:w-2/3">{{
                    $options['name'] }}</div>
                    <div class="justify-end w-1/4 text-sm font-bold text-[#2453df] text-end flex-nowrap lf:text-xs lf:w-1/3">{{
        ...
        <div>
            @foreach ($product->getTranslation('options', app()->getLocale()) as $options)
                <div class="xs:flex xs:flex-row xs:items-center xs:w-full xs:px-5 xs:mt-2 xs:pb-2 xs:border-b xs:border-slate-100">
                    <div class="w-3/4 text-sm text-[#666] text-start lf:leading-[15px]">{{ $options['name'] }}</div>
                    <div class="w-1/4 text-sm font-bold text-[#2453df] xs:text-xs text-end flex-nowrap">{{ $options['parameter']
                    }}</div>
                </div>
            @endforeach
        </div>
